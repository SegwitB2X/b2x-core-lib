# Private Key
Represents a Segwit2X private key and is needed to be able to spend Segwit2X and sign transactions.
...
Note: The WIF (Wallet Import Format) includes information about the network and if the associated public key is compressed or uncompressed (thus the same Segwit2X address will be generated by using this format).
